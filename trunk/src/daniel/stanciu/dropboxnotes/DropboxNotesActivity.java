...
//    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		Cursor cursor = managedQuery(
            ...
            " " + NotePad.Notes.COLUMN_NAME_DELETED + " = 0 ", // Only notes which are not deleted.
...
//            }
    ...
    private AndroidAuthSession buildSession() {
        AppKeyPair appKeyPair = new AppKeyPair(DropboxAppDetails.APP_KEY, DropboxAppDetails.APP_SECRET);
        AndroidAuthSession session;

        String[] stored = getKeys();
        if (stored != null) {
        	Toast.makeText(this, "Already linked with Dropbox", Toast.LENGTH_SHORT).show();
            AccessTokenPair accessToken = new AccessTokenPair(stored[0], stored[1]);
            session = new AndroidAuthSession(appKeyPair, DropboxAppDetails.ACCESS_TYPE, accessToken);
            mAlreadyLinked = true;
        } else {
            session = new AndroidAuthSession(appKeyPair, DropboxAppDetails.ACCESS_TYPE);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.addIntentOptions(Menu.CATEGORY_ALTERNATIVE, 0, 0,
                new ComponentName(this, DropboxNotesActivity.class), null, intent, 0, null);
